    ...
    def add_llr_contributions(catalog: pd.DataFrame, protein_sequences: pd.DataFrame,
        ...
        for gene, subset in catalog.groupby("gene"):
            ...
            for idx, row in subset.iterrows():
                ...
                # ESM token indices are offset by BOS; residue position 1 maps to token index 1.
                token_pos = pos
                masked = toks.clone()
                masked[0, token_pos] = mask_idx
                with torch.no_grad():
                    out = model(masked, repr_layers=[model.num_layers])
                logits = out["logits"][0, token_pos]
                probs = logits.softmax(dim=0).log()
                llr = probs[mt_id] - probs[wt_id]
                h = out["representations"][model.num_layers][0, token_pos]
        ...
        # expand top-K dims
        contrib_values = out["llr_contributions"].dropna()
        if contrib_values.empty:
            return out
        contrib_matrix = np.array(contrib_values.tolist())
        if contrib_matrix.ndim != 2 or contrib_matrix.shape[1] == 0:
            return out
